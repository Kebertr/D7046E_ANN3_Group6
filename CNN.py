...
def add_rms_noise(speech, noise_dataset, noise_level):
    if noise_level == 0:
    ...
    #pick a random noise file from the noise dataset for train, val and test. load it, and resample it to 16kHz
    random_noise = random.choice(noise_dataset)
...
# create a new dataset class that takes in the original dataset, the noise dataset, and the noise level, and returns the noisy speech and the label id for each item in the dataset
#This is done for each ep
class NoisyDataset(Dataset): 
    def __init__(self, dataset,noise_dataset, noise_level):
    ...
    def __getitem__(self, idx):
        ...
        noisy_speech, _, _ = add_rms_noise(speech,  self.noise_dataset, self.noise_level)
        label_id = 0
...
for levels in noise_levels:
    ...
    train_noise_0 = NoisyDataset(train_dataset, train_dataset_noise, noise_level=levels) #choose which noise_dataset to use here now aswell
    val_noise_0 = NoisyDataset(val_dataset, val_dataset_noise, noise_level=levels)
    test_noise_0 = NoisyDataset(test_dataset, test_dataset_noise, noise_level=levels)
